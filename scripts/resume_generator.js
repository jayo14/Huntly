const {
  ...
  AlignmentType, BorderStyle, WidthType, LevelFormat, UnderlineType, ExternalHyperlink, ImageRun
} = require('docx');
const fs = require('fs');
...
// Color palette - dynamic accent from user
const ACCENT = (data.accent_color || "#4F46E5").replace('#', '');
          ...
          new TableCell({
            ...
            children: [
              new Paragraph({
                ...
                children: [new TextRun({ text: role, font, size: 20, bold: true, color: DARK_TEXT })]
              }),
              new Paragraph({
                spacing: { before: 0, after: 60 },
                children: [new TextRun({ text: company, font, size: 19, color: MUTED, bold: true })]
              })
            ]
          }),
          new TableCell({
  ...
  if (link) {
    ...
    children.push(
      new ExternalHyperlink({
        link,
        children: [new TextRun({ text: link, font, size: 18, color: ACCENT, underline: { type: UnderlineType.SINGLE } })]
      })
    );
  }
  return new Paragraph({
            ...
            new TableCell({
                ...
                children: [
                    new Paragraph({
                        spacing: { before: 0, after: 80 },
                        children: [
                            data.linkedin ? new ExternalHyperlink({ link: data.linkedin, children: [new TextRun({ text: "LinkedIn", font, size: 17, color: ACCENT, underline: { type: UnderlineType.SINGLE } })] }) : new TextRun(""),
                            data.linkedin && data.github ? new TextRun({ text: " | ", font, size: 17, color: MUTED }) : new TextRun(""),
                            data.github ? new ExternalHyperlink({ link: data.github, children: [new TextRun({ text: "GitHub", font, size: 17, color: ACCENT, underline: { type: UnderlineType.SINGLE } })] }) : new TextRun(""),
                            (data.linkedin || data.github) && data.portfolio ? new TextRun({ text: " | ", font, size: 17, color: MUTED }) : new TextRun(""),
                            data.portfolio ? new ExternalHyperlink({ link: data.portfolio, children: [new TextRun({ text: "Portfolio", font, size: 17, color: ACCENT, underline: { type: UnderlineType.SINGLE } })] }) : new TextRun(""),
                        ]
                    })
                ]
            })
...
if (data.passport_photo_path && fs.existsSync(data.passport_photo_path)) {
    // We use a table to place photo on the right
                    ...
                    new TableCell({
                        ...
                        children: [
                            new Paragraph({
                                ...
                                children: [
                                    new ImageRun({
                                        data: fs.readFileSync(data.passport_photo_path),
